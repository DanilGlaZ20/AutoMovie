...
namespace Auto.Data
{
    public class AutoCsvFileDatabase : IAutoDatabase
    {
        ...
        private void ReadOwnersFromCsvFile(string filename)
        {
            ...
            foreach (var line in File.ReadLines(filePath))
            {
                ...
                var owner = new Owner
                {
                    FirstName = tokens[0],
                    LastName = tokens[1],
                    Age = Convert.ToInt32(tokens[2]),
                    Town= tokens[3],
                    Address= tokens[4],
                    
                };

                owners[owner.GetDriverLicence] = owner;
            }
        }
        ...
        public Owner FindOwnerByDriverLicence(string driverLicence) =>
            owners.FirstOrDefault(e => (e.Value as Owner).GetDriverLicence == driverLicence).Value;

        public Owner FindOwnerByTown(string town) =>
            owners.GetValueOrDefault(town);
        ...
        {
            owners.Add(owner.GetDriverLicence, owner);
        }

        public void UpdateOwner(Owner owner, string name)
        {
            owners[owner.GetDriverLicence] = owner;
            if (name != owner.GetDriverLicence)
            {
                owners.Remove(name);
            }
        }
        ...
        {
            owners.Remove(owner.GetDriverLicence);
        }
    }
}